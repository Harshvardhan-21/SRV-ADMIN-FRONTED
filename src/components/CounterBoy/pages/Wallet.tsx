    ...
    <div style={{ padding: '28px 32px', maxWidth: 1400 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: C.muted, fontSize: 14 }}>Track counter boy wallet transactions</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 22 }}>
        {[{ label: 'Total Credits', value: credits, color: '#065F46', bg: '#D1FAE5', Icon: ArrowDownLeft }, { label: 'Total Debits', value: debits, color: '#991B1B', bg: '#FEE2E2', Icon: ArrowUpRight }, { label: 'Net Balance', value: credits - debits, color: '#1D4ED8', bg: '#EFF6FF', Icon: Wallet }].map(card => (
          <div key={card.label} style={{ background: C.card, borderRadius: 14, padding: '16px 18px', border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 42, height: 42, borderRadius: 12, background: card.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: card.color }}><card.Icon size={20} /></div>
            <div><div style={{ fontSize: 22, fontWeight: 800, color: C.text }}>₹{card.value.toLocaleString('en-IN')}</div><div style={{ fontSize: 12, color: card.color, fontWeight: 700 }}>{card.label}</div></div>
          </div>
        ))}
      </div>
            ...
            <tbody>
              {filtered.length === 0 ? <tr><td colSpan={9} style={{ padding: 40, textAlign: 'center', color: C.muted }}>No transactions found</td></tr> : filtered.map(row => (
                <tr key={row.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                  ...
                  <td style={{ padding: '13px 16px', fontSize: 14, fontWeight: 800, color: row.type === 'credit' ? '#10B981' : '#EF4444' }}>{row.type === 'credit' ? '+' : '-'}₹{Number(row.amount).toLocaleString('en-IN')}</td>
                  <td style={{ padding: '13px 16px', fontSize: 13, fontWeight: 700, color: C.text }}>₹{Number(row.balanceAfter).toLocaleString('en-IN')}</td>
                  <td style={{ padding: '13px 16px', fontSize: 12, color: C.muted }}>{new Date(row.createdAt).toLocaleDateString('en-IN')}</td>
                </tr>
              ))}
            </tbody>
          </table>
