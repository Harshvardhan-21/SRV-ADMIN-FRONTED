...
export default function TopCounterBoys() {
  ...
  useEffect(() => {
    counterboyApi.getAll({ limit: '500' })
  ...
  const topRows = useMemo(() => {
    const pick = (row: CounterBoy) => sortBy === 'points' ? row.totalPoints ?? 0 : sortBy === 'scans' ? row.totalScans ?? 0 : row.totalRedemptions ?? 0;
    return [...rows]
  ...
  const maxValue = topRows.length ? (sortBy === 'points' ? topRows[0].totalPoints ?? 0 : sortBy === 'scans' ? topRows[0].totalScans ?? 0 : topRows[0].totalRedemptions ?? 0) : 1;

  return (
    <div style={{ padding: '28px 32px', maxWidth: 1100 }}>
      <ExportModal show={showExport} onClose={() => setShowExport(false)} title="Top Counter Boys" fileName="top-counter-boys" getData={() => topRows.map((row, index) => ({ Rank: index + 1, Name: row.name, Code: row.counterboyCode, Dealer: row.dealerName ?? '', City: row.city ?? '', State: row.state ?? '', Points: row.totalPoints ?? 0, Scans: row.totalScans ?? 0, Redemptions: row.totalRedemptions ?? 0, WalletBalance: row.walletBalance ?? 0 }))} />
      ...
      <div style={{ background: C.card, borderRadius: 14, padding: '16px 20px', border: `1px solid ${C.border}`, marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', gap: 8 }}>
          {(['points', 'scans', 'redemptions'] as SortBy[]).map(item => (
      ...
      <div style={{ background: C.card, borderRadius: 14, border: `1px solid ${C.border}`, overflow: 'hidden' }}>
        ...
        {loading ? <div style={{ padding: 40, textAlign: 'center', color: C.muted }}>Loading...</div> : topRows.length === 0 ? <div style={{ padding: 40, textAlign: 'center', color: C.muted }}>No counter boys found</div> : topRows.map((row, index) => {
          const value = sortBy === 'points' ? row.totalPoints ?? 0 : sortBy === 'scans' ? row.totalScans ?? 0 : row.totalRedemptions ?? 0;
          ...
          return (
            <div key={row.id} style={{ padding: '14px 20px', borderBottom: index < topRows.length - 1 ? `1px solid ${C.border}` : 'none', display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#F5F3FF', color: '#7C3AED', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800 }}>{row.name[0]}</div>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{row.name}</span>
                  <span style={{ fontSize: 11, color: C.muted }}>{row.counterboyCode}</span>
                </div>
                <div style={{ fontSize: 11, color: C.muted, marginBottom: 6 }}>{row.dealerName ?? 'Unassigned dealer'} • {[row.city, row.state].filter(Boolean).join(', ') || 'No location'}</div>
