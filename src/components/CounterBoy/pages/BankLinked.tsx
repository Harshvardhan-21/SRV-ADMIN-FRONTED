    ...
    <div style={{ padding: '28px 32px', maxWidth: 1400 }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#0284C7,#0369A1)', borderRadius: 18, padding: '22px 28px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {[{ label: 'Total', value: rows.length }, { label: 'Linked', value: linkedCount }, { label: 'Not Linked', value: rows.length - linkedCount }].map(card => <div key={card.label} style={{ textAlign: 'center', padding: '10px 16px', background: 'rgba(255,255,255,0.12)', borderRadius: 12 }}><div style={{ fontSize: 20, fontWeight: 900, color: 'white' }}>{card.value}</div><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.7)' }}>{card.label}</div></div>)}
        </div>
      </div>
      ...
      <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.border}`, overflow: 'hidden' }}>
        {loading ? <div style={{ padding: 40, textAlign: 'center', color: C.muted }}>Loading...</div> : <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>{['Counter Boy', 'Code', 'Phone', 'Dealer', 'Bank Status', 'UPI / Account', 'Actions'].map(head => <th key={head} style={{ padding: '14px 16px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase' }}>{head}</th>)}</tr></thead>
          <tbody>{filtered.length === 0 ? <tr><td colSpan={7} style={{ padding: 40, textAlign: 'center', color: C.muted }}>No results found</td></tr> : filtered.map(row => <tr key={row.id} style={{ borderBottom: `1px solid ${C.border}` }}><td style={{ padding: '14px 16px', fontSize: 13, fontWeight: 700, color: C.text }}>{row.name}</td><td style={{ padding: '14px 16px', fontSize: 12, color: C.muted, fontFamily: 'monospace' }}>{row.counterboyCode}</td><td style={{ padding: '14px 16px', fontSize: 13, color: C.text }}>{row.phone}</td><td style={{ padding: '14px 16px', fontSize: 12, color: C.muted }}>{row.dealerName ?? '—'}</td><td style={{ padding: '14px 16px' }}><button onClick={() => setConfirmState({ show: true, row })} style={{ background: row.bankLinked ? '#D1FAE5' : '#FEE2E2', color: row.bankLinked ? '#065F46' : '#991B1B', border: 'none', borderRadius: 20, padding: '5px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>{row.bankLinked ? <><Check size={12} /> Linked</> : <><X size={12} /> Not Linked</>}</button></td><td style={{ padding: '14px 16px', fontSize: 12, color: C.text }}>{row.upiId ?? row.bankAccount ?? '—'}</td><td style={{ padding: '14px 16px' }}><div style={{ display: 'flex', gap: 6 }}><button onClick={() => setViewing(row)} style={{ background: '#EFF6FF', color: '#1D4ED8', border: 'none', borderRadius: 7, width: 32, height: 32, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Eye size={14} /></button><button onClick={() => setEditing({ ...row })} style={{ background: '#FFF7ED', color: '#C2410C', border: 'none', borderRadius: 7, width: 32, height: 32, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Pencil size={14} /></button></div></td></tr>)}</tbody>
        </table>}
      </div>

      {viewing && <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, zIndex: 1000 }} onClick={() => setViewing(null)}><div style={{ background: C.card, borderRadius: 16, width: 520, maxWidth: '95vw', padding: 22 }} onClick={e => e.stopPropagation()}>{[['Name', viewing.name], ['Code', viewing.counterboyCode], ['Phone', viewing.phone], ['Dealer', viewing.dealerName ?? '—'], ['Bank Linked', viewing.bankLinked ? 'Yes' : 'No'], ['UPI ID', viewing.upiId ?? '—'], ['Bank Account', viewing.bankAccount ?? '—'], ['IFSC', viewing.ifsc ?? '—'], ['Wallet Balance', `₹${(viewing.walletBalance ?? 0).toLocaleString('en-IN')}`]].map(([key, value]) => <div key={String(key)} style={{ background: C.bg, borderRadius: 10, padding: 12, fontSize: 13, marginBottom: 8 }}><strong>{key}:</strong> {value}</div>)}</div></div>}
