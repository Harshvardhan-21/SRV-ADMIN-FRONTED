...
export default function CounterBoyCart() {
  ...
  useEffect(() => {
    const fetchCartData = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/cart?userRole=counterboy`);
        if (res.ok) {
    ...
    <div style={{ padding: '28px 32px', maxWidth: 1400 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: C.muted, fontSize: 14 }}>View counter boy cart items and orders</p>
        </div>
      ...
      <ExportModal 
        ...
        title="Counter Boy Cart" 
        fileName="counterboy-cart" 
