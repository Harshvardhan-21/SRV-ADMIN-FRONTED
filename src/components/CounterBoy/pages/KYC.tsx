...
type CounterBoyKYCItem = CounterBoy & {
...
function EditKYCModal({
  ...
  doc: CounterBoyKYCItem;
  onClose: () => void;
  onSave: (data: Partial<CounterBoyKYCItem>) => void;
  C: ReturnType<typeof useThemePalette>;
}) {
  const [form, setForm] = useState<Partial<CounterBoyKYCItem>>({
    aadharFrontImage: doc.aadharFrontImage ?? '',
    kycStatus: doc.kycStatus,
    kycRejectionReason: doc.kycRejectionReason ?? '',
  });

  const setField = (key: keyof CounterBoyKYCItem, value: unknown) => setForm(prev => ({ ...prev, [key]: value }));
...
export default function CounterBoyKYC() {
  ...
  const [documents, setDocuments] = useState<CounterBoyKYCItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [filterStatus, setFilterStatus] = useState('all');
  const [selectedDoc, setSelectedDoc] = useState<CounterBoyKYCItem | null>(null);
  const [editingDoc, setEditingDoc] = useState<CounterBoyKYCItem | null>(null);
  ...
  const filtered = documents.filter(item => {
    ...
    const matchSearch = !search || item.name.toLowerCase().includes(q) || item.counterboyCode.toLowerCase().includes(q) || item.phone.includes(search);
  ...
  const handleVerify = (doc: CounterBoyKYCItem) => {
    setConfirmState({
      show: true,
      title: 'Verify KYC',
      ...
      onConfirm: async () => {
        try {
          await counterboyApi.update(doc.id, { kycStatus: 'verified', kycRejectionReason: null, panNumber: null, panDocument: null });
  ...
  const handleReject = (doc: CounterBoyKYCItem) => {
    setConfirmState({
      show: true,
      title: 'Reject KYC',
      ...
      onConfirm: async () => {
        try {
          await counterboyApi.update(doc.id, { kycStatus: 'rejected', panNumber: null, panDocument: null });
  ...
  const handleDelete = (doc: CounterBoyKYCItem) => {
    setConfirmState({
      show: true,
      ...
      onConfirm: async () => {
        try {
          await counterboyApi.update(doc.id, {
    ...
    <div style={{ padding: '28px 32px', maxWidth: 1400 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: C.muted, fontSize: 14 }}>Verify and manage counter boy Aadhaar KYC documents</p>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>
                {['Counter Boy', 'Code', 'Aadhaar Doc', 'KYC Status', 'Actions'].map(head => (
            ...
            <tbody>
              {sorted.length === 0 ? (
                <tr><td colSpan={5} style={{ padding: 40, textAlign: 'center', color: C.muted }}>No counter boys found</td></tr>
              ) : sorted.map(doc => {
                ...
                return (
                  <tr key={doc.id} style={{ borderBottom: `1px solid ${C.border}` }} onMouseEnter={e => { e.currentTarget.style.background = C.hoverRow; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}>
                    ...
                    <td style={{ padding: '13px 16px', fontSize: 12, color: C.muted, fontFamily: 'monospace' }}>{doc.counterboyCode}</td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', backdropFilter: 'blur(6px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setSelectedDoc(null)}>
          <div style={{ background: C.card, borderRadius: 16, width: 580, maxWidth: '95vw', boxShadow: '0 25px 70px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: 22, display: 'grid', gap: 10 }}>
              {[['Code', selectedDoc.counterboyCode], ['KYC Status', selectedDoc.kycStatus], ['Rejection Reason', selectedDoc.kycRejectionReason || '-']].map(([key, value]) => (
