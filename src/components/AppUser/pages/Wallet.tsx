...
export default function UserWallet() {
  ...
  useEffect(() => {
    walletApi.getTransactions({ userRole: 'user', limit: '500' })
      .then(res => setRows(Array.isArray(res) ? res : (res as any).data ?? []))
      .catch(console.error)
      .finally(() => setLoading(false));
  }, []);

  const filtered = rows.filter(row => ((row.userId ?? '').toLowerCase().includes(search.toLowerCase()) || (row.description ?? '').toLowerCase().includes(search.toLowerCase())) && (filterType === 'all' || row.type === filterType));
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1400 }}>
      <ExportModal show={showExport} onClose={() => setShowExport(false)} title="User Wallet" fileName="user-wallet" getData={() => rows.map(row => ({ UserId: row.userId, Type: row.type, Source: row.source, Description: row.description, Amount: row.amount, BalanceAfter: row.balanceAfter, Date: row.createdAt }))} />
