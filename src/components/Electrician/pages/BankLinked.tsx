...
function ViewModal({ el, onClose, C }: { el: Electrician; onClose: () => void; C: any }) {
  const tier = TIER_CONFIG[el.tier as MemberTier] ?? TIER_CONFIG['Silver'];
  return (
      ...
      <div style={{ background: C.card, borderRadius: 20, width: 520, maxWidth: '95vw', boxShadow: '0 25px 70px rgba(0,0,0,0.25)', border: `1px solid ${C.border}` }} onClick={e => e.stopPropagation()}>
        <div style={{ background: C.heroGradient, padding: '22px 26px', borderRadius: '20px 20px 0 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: `linear-gradient(135deg, ${C.red}, ${C.redDark})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 800, color: 'white' }}>
              {el.name[0]}
            </div>
            <div>
              <div style={{ fontSize: 18, fontWeight: 800, color: 'white' }}>{el.name}</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)' }}>{el.electricianCode}</div>
            </div>
          </div>
        ...
        <div style={{ padding: 24, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {[
            ...
            { label: 'Total Points', value: el.totalPoints.toLocaleString('en-IN') },
          ].map(item => (
...
function EditModal({ el, onClose, onSave, C }: { el: Electrician; onClose: () => void; onSave: (e: Electrician) => void; C: any }) {
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', backdropFilter: 'blur(6px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
      <div style={{ background: C.card, borderRadius: 20, width: 560, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 25px 70px rgba(0,0,0,0.25)', border: `1px solid ${C.border}` }} onClick={e => e.stopPropagation()}>
        <div style={{ padding: '20px 24px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 17, fontWeight: 800, color: C.text }}>Edit Details — {el.name}</div>
        ...
        <div style={{ padding: '20px 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {/* Personal */}
          <div style={{ gridColumn: '1/-1', fontSize: 12, fontWeight: 700, color: C.muted, textTransform: 'uppercase', paddingBottom: 6, borderBottom: `1px solid ${C.border}` }}>Personal Info</div>
          <div>
          ...
          <div>
            ...
            <input style={inputStyle} value={form.dealerName} onChange={e => f('dealerName', e.target.value)} />
          </div>
          {/* Bank */}
          ...
          <div style={{ gridColumn: '1/-1', display: 'flex', alignItems: 'center', gap: 12 }}>
            <input id="bank-linked" type="checkbox" checked={form.bankLinked} onChange={e => f('bankLinked', e.target.checked)} style={{ width: 18, height: 18, cursor: 'pointer' }} />
            <label htmlFor="bank-linked" style={{ fontSize: 14, fontWeight: 700, color: C.text, cursor: 'pointer' }}>Bank Account Linked</label>
          </div>
          <div>
          ...
          <div>
            ...
            <input style={inputStyle} value={form.ifsc ?? ''} onChange={e => f('ifsc', e.target.value)} placeholder="e.g. SBIN0001234" />
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 700, color: C.muted, display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>Wallet Balance (₹)</label>
            <input type="number" style={inputStyle} value={form.walletBalance ?? ''} onChange={e => f('walletBalance', e.target.value === '' ? '' : Number(e.target.value))} />
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 700, color: C.muted, display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>Total Points</label>
            <input type="number" style={inputStyle} value={form.totalPoints ?? ''} onChange={e => f('totalPoints', e.target.value === '' ? '' : Number(e.target.value))} />
          </div>
        </div>
...
export default function ElectricianBankLinked() {
  ...
  const filtered = data.filter(e => {
    ...
    const matchSearch = e.name.toLowerCase().includes(q) || e.phone.includes(q) || e.electricianCode.toLowerCase().includes(q) || e.city.toLowerCase().includes(q);
    const matchBank = filterBank === 'all' || (filterBank === 'linked' ? e.bankLinked : !e.bankLinked);
    const matchTier = filterTier === 'all' || e.tier === filterTier;
    return matchSearch && matchBank && matchTier;
  });

  const linked = data.filter(e => e.bankLinked).length;
  const notLinked = data.filter(e => !e.bankLinked).length;
  ...
  const confirmToggle = async () => {
    try {
      await electricianApi.update(confirmState.id, { bankLinked: !confirmState.linked });
      setData(prev => prev.map(e => e.id === confirmState.id ? { ...e, bankLinked: !confirmState.linked } : e));
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1400 }}>
      ...
      <ExportModal show={showExport} onClose={() => setShowExport(false)} title="Electrician Bank Details" fileName="electrician-bank" getData={() => filtered.map(e => ({ Name: e.name, Code: e.electricianCode, Phone: e.phone, City: e.city, Tier: e.tier, BankLinked: e.bankLinked ? 'Yes' : 'No', UPI: e.upiId ?? '', BankAccount: e.bankAccount ?? '', IFSC: e.ifsc ?? '' }))} />
      ...
      <div style={{ background: 'linear-gradient(135deg, #0284C7, #0369A1)', borderRadius: 18, padding: '22px 28px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 8px 24px rgba(2,132,199,0.25)' }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.75)', marginTop: 4 }}>Manage electrician bank account & UPI details</div>
        </div>
